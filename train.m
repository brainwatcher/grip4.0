...
%     session= str2double(subinfo{2});
    ...
    %% train begin
    train_begin_img=imresize(train0,min(rect(3)/size(bpm_prepare,2),rect(4)/size(bpm_prepare,1)));
    imageDisplay8=Screen('MakeTexture', window, train_begin_img);
    %% train end
    train_end_img=imresize(train1,min(rect(3)/size(bpm_prepare,2),rect(4)/size(bpm_prepare,1)));
    ...
    for w=1:block
        ...
        for j=1:trial_num(w)
            ...
            while GetSecs-rt0{w}{j}<max_time % the duration of no-response trial
                ...
                time(i)=GetSecs;
                mark=check_end(path,time);
                i=i+1;
                if  current_cursor>gate(4,1)&& mark==1% end of the trial ;% note the 4 to 4.5
                    rt1{w}{j}=GetSecs;
                    Screen('DrawTexture', window, imageDisplay, [], [],0);
                    Screen('DrawTexture', window, imageDisplay_stop, [], [symbol_pos,symbol_pos+symbol_size],0);
...
%                     disp(['trial ' num2str(j) ' finished']) 
            ...
            [acc,path,time,shoot] = evaluate_acc_train(path,time,ans_gate);
            time_all{w}{j}=time;
            acc_all{w}{j}=acc;
